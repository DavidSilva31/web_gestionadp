...
CREATE TABLE IF NOT EXISTS clientes (
  ...
  sector      TEXT,
  activo      BOOLEAN DEFAULT TRUE,
  dia_corte_facturacion SMALLINT NOT NULL DEFAULT 1,
  created_at  TIMESTAMPTZ DEFAULT NOW(),
  updated_at  TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS tarifas_cliente (
  ...
  clase_imo             TEXT,
  -- 'UF' (default) o 'CLP' — algunos clientes (ej. BASF) facturan en pesos
  -- fijos, no indexados a UF. Si moneda='CLP', usar las columnas _clp.
  ...
  tarifa_almacenaje_clp NUMERIC(12, 2),
  tarifa_inout_clp      NUMERIC(12, 2),
...
-- ── Vincular movimientos/reports a una tarifa específica ────────────────
-- Clientes con más de un contrato en paralelo (ej. PROQUIMIN: CARGA NORMAL,
-- CLASE IMO 3/4.2/8/9, LOSA ISOTANQUES) necesitan que cada movimiento diga a
-- cuál pertenece — de lo contrario el HES agrega TODOS los movimientos del
-- cliente sin importar la tarifa seleccionada, mezclando productos distintos
-- en cada HES. NULL = cliente de una sola tarifa (caso más común).
...
-- El trigger que auto-crea un movimiento al despachar un report propaga la
-- tarifa elegida en el report hacia el movimiento generado (redefine la
-- función creada más arriba — mismo nombre, Postgres usa esta versión).
CREATE OR REPLACE FUNCTION create_movimiento_from_report()
RETURNS TRIGGER AS $$
DECLARE
  v_tipo     TEXT;
